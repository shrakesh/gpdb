...
Feature: gpcheckcat tests
    ...
    Scenario Outline: gpcheckcat should discover missing attributes for external tables
        ...
        And the user runs "gpfdist -p 8081 -d /tmp &"
        And there is a partition table "part_external" has external partitions of gpfdist with file "backup_gpfdist_dummy" on port "8081" in "miss_attr_db3" with data
