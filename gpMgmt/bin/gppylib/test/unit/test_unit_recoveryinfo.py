    ...
    def test_run_recovery(self):
        tests = [
            ...
            },
            {
                ...
                "expected_info_msgs": [call(Contains('-----')),
                                       call('Failed to action_recover the following segments. You must run gprecoverseg --differential for all incremental failures'),
            ...
            },
            {
                ...
                "expected_info_msgs": [call(Contains('-----')),
                                       call(
                                           'Failed to action_recover the following segments. You must run gprecoverseg -F for all differential failures'),
            ...
            },
            {
                ...
                "expected_info_msgs": [call(Contains('-----')),
                                       call(Contains('Failed to action_recover the following segments. You must run gprecoverseg --differential for all incremental failures')),
                                       call(self._msg('host1', 7003, logfile='/tmp/progress3', type='incremental')),
        ...
        for test in tests:
            with self.subTest(msg=test["name"]):
                ...
                for failed_dbid in dbids_that_failed_bb_rewind:
                    self.assertFalse(r.was_bb_rewind_differential_successful(failed_dbid))
                dbids_that_passed_bb_rewind = self.all_dbids - set(dbids_that_failed_bb_rewind)
                for pass_dbid in dbids_that_passed_bb_rewind:
                    self.assertTrue(r.was_bb_rewind_differential_successful(pass_dbid))
